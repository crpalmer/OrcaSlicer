...
int ObjectTablePanel::init_filaments_and_colors()
{
    ...
    // ORCA: include mixed (virtual) filaments so the per-object/feature/support filament dropdowns list them.
    const std::vector<std::string> filament_colors = m_plater->get_extruder_colors_from_plater_config();
    m_filaments_count = int(filament_colors.size());
    ...
    for (int i = 0; i < m_filaments_count; ++i) {
        ...
        // Map the virtual index to its enabled mixed-filament row for a descriptive name.
        ...
        for (const MixedFilament &mf : wxGetApp().preset_bundle->mixed_filaments.mixed_filaments()) {
            ...
            m_filaments_name[i] = wxString::Format("%d: Mixed Filament %d (F%u + F%u)",
                                                   i + 1, i + 1, unsigned(mf.component_a), unsigned(mf.component_b));
            break;
        }
